...
def clean(doc, stop, lemma, exclude):
...
def process_data(interview_list):
    sentence_list = []
...
def dic_words_per_topic(topics):
    final_list = {}
...
def get_topics_words(num_topic, num_words, data):
    ...
    interview_list = process_data(interview_list)
    interview_list = interview_list['Interview Questions'].tolist()
    question_clean = [clean(ques, stop, lemma, exclude).split() for ques in interview_list]
    ...
    topics_dictionary = dic_words_per_topic(topics)
...
dict_topic_words = get_topics_words(1, 10, interview_questions)
