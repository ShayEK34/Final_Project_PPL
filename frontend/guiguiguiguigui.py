...
def raise_frame(frame):
    if (frame.name =="results" and company_name.get() == "google"):
        frame= Frame(root)
        left_frame = Frame(frame, width=1900, height=600, bg='#99CCCC')
        left_frame.grid(row=0, column=0, padx=17, pady=5)
        middle_frame = Frame(frame, width=200, height=200, bg='grey')
        middle_frame.grid(row=0, column=1, padx=10, pady=5)
        right_frame = Frame(frame, width=1000, height=1000, bg='grey')
        ...
        image=Image.open('resurces/google.jpg')
